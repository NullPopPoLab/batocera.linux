...
def generatePadsConfig(cfgPath, playersControllers, sysName, dpadMode, altButtons, customCfg, specialController, decorations):
    ...
    mappings = {
        ...
        "JOYSTICKLEFT_UP":    "up",
        "JOYSTICKLEFT_DOWN":  "down",
        "JOYSTICKLEFT_LEFT":  "left",
        "JOYSTICKLEFT_RIGHT": "right",
        "JOYSTICKRIGHT_UP": "x",
        "JOYSTICKRIGHT_DOWN": "b",
        "JOYSTICKRIGHT_LEFT": "y",
        "JOYSTICKRIGHT_RIGHT": "a",
        "BUTTON1": "c",
        "BUTTON2": "b",
        "BUTTON3": "a",
        "BUTTON4": "z",
    ...
    uimappings = {
        'UI_UP':{'hot':False,'pad':'up','key':'UP'},
        'UI_DOWN':{'hot':False,'pad':'down','key':'DOWN'},
        'UI_LEFT':{'hot':False,'pad':'left','key':'LEFT'},
        'UI_RIGHT':{'hot':False,'pad':'right','key':'RIGHT'},
        'UI_SELECT':{'hot':False,'pad':'b','key':'ENTER'},
        'UI_CLEAR':{'hot':False,'pad':'l3','key':'DEL'},
        'UI_CANCEL':{'hot':False,'pad':'r3','key':'BACKSPACE'},
        'UI_CONFIGURE':{'hot':False,'pad':'menu','key':'TAB'},

        'VOLUME_UP':{'hot':True,'pad':'up'},
        'VOLUME_DOWN':{'hot':True,'pad':'down'},
        'UI_REWIND_SINGLE':{'hot':True,'pad':'left'},
        'UI_FAST_FORWARD':{'hot':True,'pad':'right'},
        'UI_PAUSE':{'hot':True,'pad':'b'},
        'UI_PAUSE_SINGLE':{'hot':True,'pad':'c'},
        'UI_LOAD_STATE':{'hot':True,'pad':'x'},
        'UI_SAVE_STATE':{'hot':True,'pad':'y'},
        'UI_TOGGLE_CHEAT':{'hot':True,'pad':'z'},
        'UI_SHOW_GFX':{'hot':True,'pad':'pageup'},
        'SERVICE':{'hot':True,'pad':'l2'},
        'UI_ROTATE':{'hot':True,'pad':'r2'},
        'UI_RECORD_AVI':{'hot':True,'pad':'l3'},
        'UI_SNAPSHOT':{'hot':True,'pad':'r3'},
        'SERVICE1':{'hot':True,'pad':'select'},
        'UI_EXIT_GAME':{'hot':True,'pad':'start'},
        'UI_SOFT_RESET':{'hot':True,'pad':'menu'},
    }
...
#        xml_system_alt.appendChild(xml_input_alt)
    ...
    for playercontroller, pad in sorted(playersControllers.items()):
        ...
        hotkey = None
        if nplayer == 1:
            if 'hotkey' in pad.inputs:
                hotkey = pad.inputs['hotkey']
        
            for mapping in uimappings:
                keyname=None
                padobj=None
                padname=None
                reversed=False
                hotside=False
                if 'key' in uimappings[mapping]: keyname=uimappings[mapping]['key']
                if 'pad' in uimappings[mapping]: padname=uimappings[mapping]['pad']
                if 'hot' in uimappings[mapping]: hotside=uimappings[mapping]['hot']
        
                if padname is not None:
                    if padname in pad.inputs:
                        padobj=pad.inputs[padname]
                    else:
                        rpadname=reverseMapping(padname)
                        if rpadname in pad.inputs:
                            padname=rpadname
                            padobj=pad.inputs[rpadname]
                            reversed=true
                xml_input.appendChild(generateUIElement(config, 'standard', pad.index, mapping, keyname, padname, padobj, hotkey, hotside, reversed, dpadMode, '', ''))
...
#        else:        
        ...
        if key == "joystick2up":
            return "JOYCODE_{}_RYAXIS_NEG_SWITCH OR JOYCODE_{}_BUTTON6".format(joycode, joycode)
        if key == "joystick2down":
            return "JOYCODE_{}_RYAXIS_POS_SWITCH OR JOYCODE_{}_BUTTON2".format(joycode, joycode)
        if key == "joystick2left":
            return "JOYCODE_{}_RXAXIS_NEG_SWITCH OR JOYCODE_{}_BUTTON5".format(joycode, joycode)
        if key == "joystick2right":
            return "JOYCODE_{}_RXAXIS_POS_SWITCH OR JOYCODE_{}_BUTTON1".format(joycode, joycode)
